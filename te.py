...
# ----------------- CONFIGURATION -----------------
MODEL_PATH = "/home/user2/VOIP/finetune_checkpoints_ecapa_voip/best_model_eer_10.08.pt"  # Update with your best model path
DATASET_ROOT = "/home/user2/VOIP/VOIP_Mel_Features"
OUTPUT_DIR = "/home/user2/VOIP/ecapa_evaluation_results"  # All results will be saved here
...
def plot_eer_curve(fpr, fnr, thresholds, eer, eer_threshold, output_path):
    ...
    plt.title('Standard ECAPA-TDNN: Detection Error Tradeoff (DET) Curve - FAR vs FRR', fontsize=14, fontweight='bold')
...
def plot_roc_curve(fpr, tpr, eer, output_path):
    ...
    plt.title('Standard ECAPA-TDNN: Receiver Operating Characteristic (ROC) Curve', fontsize=14, fontweight='bold')
...
def plot_score_distributions(scores, y_true, eer_threshold, output_path):
    ...
    plt.title('Standard ECAPA-TDNN: Score Distributions - Genuine vs Impostor Pairs', fontsize=14, fontweight='bold')
...
def plot_tsne(embeddings_dict, labels_dict, output_path, n_speakers=20, n_samples_per_speaker=10):
    ...
    plt.title(f'Standard ECAPA-TDNN: t-SNE Visualization of Speaker Embeddings\n({n_speakers} speakers, {n_samples_per_speaker} samples each)', 
...
def plot_det_curve(fpr, fnr, eer, output_path):
    ...
    plt.title('Standard ECAPA-TDNN: Detection Error Tradeoff (DET) Curve', fontsize=14, fontweight='bold')
...
def save_metrics_to_csv(eer, min_dcf, thresholds_at_eer, thresholds_at_minDCF, output_path):
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("STANDARD ECAPA-TDNN EVALUATION METRICS\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    """Main evaluation function"""
    logging.info("=" * 60)
    logging.info("STARTING EVALUATION OF STANDARD ECAPA-TDNN")
    logging.info("=" * 60)
...
def create_summary_report(eer, min_dcf, num_samples, output_dir):
    ...
    with open(report_path, 'w') as f:
        f.write("# Standard ECAPA-TDNN Evaluation Report\n\n")
        f.write(f"**Evaluation Date:** {pd.Timestamp.now()}\n\n")
        f.write(f"**Model:** {MODEL_PATH}\n\n")
